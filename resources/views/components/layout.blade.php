...
<head>
    ...
    <title>{{ $title ?? 'Portofolio Devanno' }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body class="font-sans antialiased text-gray-900 bg-slate-50 dark:bg-slate-900 dark:text-white transition-colors duration-300">
    
    <x-navbar />

    <button @click="darkMode = !darkMode" class="fixed bottom-6 right-6 z-50 p-3 rounded-full bg-slate-800 dark:bg-yellow-400 text-white dark:text-slate-900 shadow-lg hover:scale-110 transition-transform duration-200">
